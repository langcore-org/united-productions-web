#!/usr/bin/env node
/**
 * Feature Cleanup - Delete Feature Script (Auto-generated template)
 * 
 * 機能削除のための統合スクリプト
 * - ファイル削除
 * - コード参照削除
...
function detectCodeRefs(featureId) {
  ...
  for (const path of searchPaths) {
    ...
    const result = runCommand(`grep -rn "${featureId}" ${path} --include="*.ts" --include="*.tsx" 2>/dev/null`);
    if (result.trim()) {
      result.trim().split('\n').forEach(line => {
...
async function detectDbData(featureId) {
  ...
  // FeaturePrompt
  const featurePrompts = await prisma.featurePrompt.findMany({
    where: { featureId }
  });
  data.featurePrompts = featurePrompts;
  
  // SystemPrompt（関連キーを推測）
  const possibleKeys = [
...
async function main() {
  ...
  if (!onlyDb) {
    ...
    if (filesToDelete.files.length === 0 && filesToDelete.dirs.length === 0) {
      ...
      logItem('info', `合計: ${filesToDelete.files.length}ファイル, ${filesToDelete.dirs.length}ディレクトリ`);
    }
  }
  ...
  if (!onlyFiles && (dbData.featurePrompts.length > 0 || dbData.systemPrompts.length > 0)) {
    ...
    try {
      ...
      if (dbData.featurePrompts.length > 0) {
        const result = await prisma.featurePrompt.deleteMany({
          where: { featureId }
        });
        logItem('success', `FeaturePrompt: ${result.count}件削除`);
      }
      
      // SystemPromptVersionとSystemPrompt削除
      for (const prompt of dbData.systemPrompts) {
        const versionsResult = await prisma.systemPromptVersion.deleteMany({
          where: { promptId: prompt.id }
        });
        logItem('success', `SystemPromptVersion (${prompt.key}): ${versionsResult.count}件削除`);
