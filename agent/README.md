...
- **TodoWrite streaming** - Real-time task progress updates via SSE 🆕
- **Show thinking mode** - Expose agent reasoning process when enabled 🆕
...
## TodoWrite Streaming & Show Thinking 🆕

The wrapper supports **real-time task progress** and **agent thinking visibility** through SSE streaming.

### TodoWrite Streaming

When Claude Code uses the `TodoWrite` tool to track tasks, these updates are streamed to clients in real-time:

```bash
# SSE stream includes todo_update events
data: {"type": "todo_update", "todos": [
...
Enable `show_thinking` to see Claude's reasoning process:
...
response = client.chat.completions.create(
    ...
    extra_body={
        "enable_tools": True,
        "show_thinking": True  # Expose agent thinking
    },
    stream=True
)
```

When enabled, the stream includes tool usage and internal reasoning steps.

### Frontend Integration Example

For Vercel AI SDK (`useChat` hook), todo updates come through the `data` stream:

```typescript
const { data } = useChat({ api: '/api/chat' });

// Parse todo updates from data stream
useEffect(() => {
  if (data) {
    const todoUpdate = data.find(d => d.type === 'todo_update');
    if (todoUpdate) {
...
- **✅ TodoWrite Streaming**: Real-time task progress via SSE events 🆕
- **✅ Show Thinking Mode**: Optional agent reasoning visibility 🆕
