...
│   ├── programToPromptTextSimple()    # 簡易版変換
│   ├── programToPromptTextDetailed()  # 詳細版変換
│   ├── createSystemPromptFooter()     # フッター生成
│   ├── createSingleProgramPromptBase() # 単一番組生成
│   ├── createAllProgramsPromptBase()   # 全番組生成
│   └── createCompositeSystemPrompt()   # 複合プロンプト生成
│
...
lib/llm/
├── prompt-builder.ts     # API用プロンプト構築（featureId対応）
│   ├── FEATURE_TO_PROMPT_KEY  # 機能IDとプロンプトキーのマッピング
│   ├── getPromptKeyForFeature()  # 機能IDからプロンプトキーを取得
│   ├── isValidFeatureId()        # 機能IDの検証
│   └── buildSystemPrompt()       # システムプロンプト構築
│
└── ...
...
```sql
-- supabase/migrations/ 内で定義
CREATE TABLE system_prompts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  key TEXT UNIQUE NOT NULL,        -- "RESEARCH_CAST", "PROPOSAL" 等
  name TEXT NOT NULL,              -- 表示名
  description TEXT,                -- 説明
  content TEXT NOT NULL,           -- プロンプト本文
  category TEXT NOT NULL,          -- "research", "minutes", "schedule" 等
...
### FeaturePrompt テーブル（マッピングテーブル）

コード内で定義（`lib/llm/prompt-builder.ts`）:
...
**注**: 以前はDBテーブルでしたが、コード内定数に移行しました。新機能追加時は `FEATURE_TO_PROMPT_KEY` にエントリを追加してください。
...
| ファイル | 役割 |
|---------|------|
| `lib/knowledge/system-prompt.ts` | システムプロンプト生成の共通ロジック |
| `lib/llm/prompt-builder.ts` | API用プロンプト構築 |
| `lib/prompts/db/crud.ts` | DBからプロンプトを取得 |
...
| ドキュメント | 内容 |
|-------------|------|
| [system-prompt-management.md](./system-prompt-management.md) | システムプロンプト管理（DB管理・バージョン履歴） |
...
| [prompt-management.md](./prompt-management.md) | プロンプト管理仕様 |
