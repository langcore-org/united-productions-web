...
実装中に「後で対応したい」タスクや問題を発見したら、**必ず `docs/backlog/` に記録する**。

### 記録が必要なもの

| 種別 | 例 | 優先度 |
|-----|------|-------|
| **保留タスク** | エラーハンドリングの改善、リファクタリング | ⏸️ 保留 |
| **技術的負債** | パフォーマンス問題、コードの重複 | 🔴 高 / 🟡 中 |
| **バグの温床** | 不安定な実装、エッジケース未対応 | 🔴 高 |
| **調査が必要** | 新ライブラリの検討、設計の見直し | 🟡 中 / 🟢 低 |
...
```typescript
// TODO: [高] エラーハンドリングの改善
// docs/backlog/todo-error-handling-cleanup.md を参照
// 現在は簡易的な実装
```
...
詳細: [docs/backlog/README.md](../../../backlog/README.md)
