...
Vercelデプロイ後にGoogle認証で以下のエラーが発生した場合の解決手順です：
...
6. [手順4: 本番公開（必要に応じて）](#手順4-本番公開必要に応じて)
...
Vercelにデプロイ後、ドメインが変わるため、Google Cloud Consoleで設定が必要になります。
...
| `email` | メールアドレス取得 |
| `profile` | プロフィール情報取得 |
| `https://www.googleapis.com/auth/drive.readonly` | Google Drive読み取り（必要な場合） |
...
> 設定画面を素早く開くヘルパー: `./scripts/open-oauth-consent.sh [PROJECT_ID]`
...
VercelのドメインをGoogle OAuthに登録する必要があります。
...
| 本番（Vercel） | `https://agent1-brown.vercel.app/api/auth/callback/google` |

> **注意**: ドメインが異なる場合（カスタムドメインなど）、適切なURIに置き換えてください。
...
2. Vercelのドメイン（`https://your-app.vercel.app/api/auth/callback/google`）が正確に登録されているか確認
...
2. Vercelの環境変数 `GOOGLE_CLIENT_ID` と `GOOGLE_CLIENT_SECRET` を確認

### ログイン後にアプリに戻らない

**原因**: `NEXTAUTH_URL` が正しく設定されていない

**解決策**:
1. Vercelの環境変数 `NEXTAUTH_URL` を確認
2. 値は `https://your-app.vercel.app` である必要があります
3. 変更後は再デプロイが必要
...
- [ ] Google Cloud Console → 認証情報 → 承認済みリダイレクトURIにVercelドメインを追加
- [ ] Vercel → 環境変数 → `NEXTAUTH_URL` が本番URLに設定されている
- [ ] ブラウザキャッシュをクリアして再試行
