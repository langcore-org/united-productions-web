...
2026-02-20 にダークモードからライトモードへの移行を実施。移行は不完全で、以下の問題が残存している：

- チャットUIに **ダークモードの色が4箇所残存**しており、視覚的バグとなっている
- 色の指定がコンポーネントに分散（`#ff6b00`、`bg-[#1e1e24]` など）
- `globals.css` にチャット用 `--color-grok-*` 変数が定義済みだが、チャットUIが一切使っていない
- 将来的なテーマ変更（ダークモード復活など）に備えた統一構造がない
...
ライトモード移行後も以下の箇所にダークモード色が残っており、白背景の中に暗色が混入している：

| ファイル | 行 | 問題の色 | 本来あるべき色 |
|---------|-----|---------|--------------|
| `components/ui/FeatureChat.tsx` | 258 | `bg-[#0d0d12]` | `bg-white` |
| `components/ui/FeatureChat.tsx` | 464 | `bg-[#2a2a35]` | `bg-gray-200` |
| `components/ui/MessageBubble.tsx` | 162 | `bg-[#0d0d12] border-[#2a2a35]` | `bg-gray-50 border-gray-200` |
| `components/ui/ModelSelector.tsx` | 125 | `hover:bg-[#2a2a35]` | `hover:bg-gray-100` |
...
|--------|---------|------|--------|------|------|
| A | ダークモード残存バグ修正 | 極小 | **緊急** | 大 | 4箇所のみ。今すぐやる |
...
1. **ダークモード残存の4箇所を修正**（優先度A）
   - `FeatureChat.tsx:258` → `bg-white`
   - `FeatureChat.tsx:464` → `bg-gray-200 text-gray-400`
   - `MessageBubble.tsx:162` → `bg-gray-50 border-gray-200`
   - `ModelSelector.tsx:125` → `hover:bg-gray-100`

2. **目視確認**
   - 各コンポーネントを開いて暗色が残っていないか確認
