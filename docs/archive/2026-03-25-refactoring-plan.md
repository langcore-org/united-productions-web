...
> **概要**: Teddyコードベース全体のリファクタリング計画（2026年3月23日作成）
> **現状**: 主要タスクの多くが完了し、残りはRAG実装後や4月以降に順次対応予定
> **ステータス**: ⏸️ **保留（計画凍結）** — 完了済み項目を記録し、未対応項目は別途計画書を作成して対応
> **作成日**: 2026-03-23
> **最終更新**: 2026-03-25
> **対象**: Teddy コードベース全体
> **前提**: 3/31正式リリース（残り6日）、4月以降エージェントループ + RAG実装予定
...
| 優先度 | 項目 | 完了日 |
|--------|------|--------|
| S | CSP設定の強化 | 2026-03-24 |
| S | Supabase RLSポリシー確認 | 2026-03-24 |
| A | grok.ts / grok-refactored.ts 統一 | 2026-03-22 |
| A | 不要な依存関係の削除 | 2026-03-22 |
| A | デッドコード削除 | 2026-03-22 |
| B | FeatureChat.tsx の責務分割 | 2026-03-24 |
| B | useLLMStream の状態グループ化 | 2026-03-24 |
| B | エラーレスポンスフォーマットの統一 | 2026-03-24 |

### 保留中項目（⏸️ 別途計画書を作成して対応）

| 優先度 | 項目 | 保留理由 | 想定対応時期 |
|--------|------|----------|-------------|
| A-4 | APIルートの認証パターン統一 | 4月エージェントループ実装と並行して対応 | 2026年4月第1週 |
| B-2 | ページコンポーネントの肥大化対策（admin/*） | 管理画面は現在使用者がおらず優先度が低い | 必要に応じて |
| C-1 | programs-detailed-data の整理 | RAG実装でデータ構造が変わる可能性があるため | RAG設計確定後 |
| C-2 | テストカバレッジの拡大 | リリース後に継続的に追加 | 継続的に実施 |
| C-3 | system-prompt.ts のデータ分離 | RAG実装時にデータソースが変わる可能性がある | RAG実装時 |
| C-4 | next.config.ts の `ignoreBuildErrors: true` 解除 | 型エラーを全て解消してから有効化 | 型エラー解消後 |
...
| **工数** | 4-6h |
| **判断** | ⏸️ **保留** — エージェントループ実装時に別途計画書を作成して対応 |
...
| **リスク** | 低。データの移動のみ |
| **工数** | 1-2h |
| **判断** | ⏸️ **保留** — RAG実装でデータ構造が変わる可能性があるため、RAG設計確定後に対応 |
...
| **リスク** | なし |
| **工数** | 8-12h（下記4箇所） |
| **判断** | ⏸️ **保留** — リリース後に継続的に追加。リファクタリング実施時にセットで対応 |
...
| **リスク** | 低。データ参照パスの変更のみ |
| **工数** | 1-2h |
| **判断** | ⏸️ **保留** — RAG実装時にデータソースが変わる可能性があるため、その時まで待つ方が効率的 |
...
【リリース直後（4月第1週）】✅ 完了
  A-1. grok.ts / grok-refactored.ts 統一          ✅ 2026-03-22
  A-2. 不要依存関係の削除                          ✅ 2026-03-22
  A-3. デッドコード削除                            ✅ 2026-03-22

【完了済み（3/24）】✅
  B-1. FeatureChat分割                             ✅ 2026-03-24
  B-3. useLLMStream状態グループ化                  ✅ 2026-03-24
  B-4. エラーフォーマット統一                      ✅ 2026-03-24

【保留（エージェントループ実装時に別途計画）】⏸️
  A-4. APIパターン統一                             ⏸️ 保留
...
【保留（RAG実装時に対応）】⏸️
  C-1. programs-detailed-data整理                  ⏸️ 保留
  C-3. system-prompt.tsデータ分離                  ⏸️ 保留
  C-4. ignoreBuildErrors解除                       ⏸️ 保留
```
...
|--------|---------|-----------|
| **S（セキュリティ）** | 4-7h | ✅ 完了 |
| **A（高優先度）** | 2.5-3.5h | ✅ 完了（A-4は保留） |
| **B（中優先度）** | 5-7h | ✅ 完了（B-2は保留） |
| **C（低優先度）** | — | ⏸️ 保留 |
| **合計（完了）** | **11.5-17.5h** | |
...
## 6. 今後の対応方針（2026-03-25）

### 保留項目の対応タイミング

| 項目 | 対応タイミング | 備考 |
|------|---------------|------|
| A-4 APIルートの認証パターン統一 | エージェントループ実装時 | 別途詳細計画書を作成 |
| B-2 ページコンポーネント分割 | 管理画面使用開始時 | 現状優先度低 |
| C-1, C-3 データ整理 | RAG実装時 | データ構造確定後 |
| C-2 テストカバレッジ | 継続的に | リファクタリング時にセットで |
| C-4 ignoreBuildErrors解除 | 型エラー解消後 | 急がない |

### 計画書作成時の参照ポイント

- 本計画書の「詳細分析データ」セクション（4.1〜4.5）
- 各保留項目の「現状の2パターン」など詳細な問題分析
- 完了済み項目の対応実績と教訓

---

*最終更新: 2026-03-25（計画凍結・archiveへ移動）*
