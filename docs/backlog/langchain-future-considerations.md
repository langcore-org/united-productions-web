...
### 重要: 現在の技術スタック（2026-03-20）

| 項目 | 現在の実装 |
|------|-----------|
| **LLMプロバイダー** | **xAI (Grok) のみ** |
| **実装方式** | **xAI直接API呼び出し**（LangChain不使用） |
| **LangChain** | 将来のGemini追加に備え保持（現在使用せず） |

**なぜLangChainを保持するのか**: 将来Google Geminiを追加する際にLangChain経由で統合できるように、パッケージとコードを保持しています。詳細は [AGENTS.md](../../AGENTS.md) を参照してください。
