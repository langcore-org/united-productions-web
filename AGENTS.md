...
| **specs/ を最優先** | 実装と仕様が矛盾したら `specs/` を正とする |
| **current/ のみ参照** | `plans/current/` 以外の計画書は参照しない（archive/は過去のもの） |
| **完了した計画は archive/ へ** | 計画が完了したら `current/` から `archive/` へ移動 |
