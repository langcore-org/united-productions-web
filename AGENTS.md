...
| **「プロンプトを本番DBに反映する」** | 本項の **「📝 プロンプト管理」→「プロンプトをDBに反映する」** 参照 | `docs/prompts/*.md` 編集後は `node scripts/prompts/update-from-doc.mjs <KEY> "理由"` |
...
| 状況 | 対応 |
|------|------|
| **「AIの応答に思考過程（「〜について考えます」等）が含まれる」** | `docs/prompts/` の該当プロンプトに「思考プロセスを出力に含めない」を追加し、下記「プロンプトをDBに反映する」で反映 |
| **「AIの応答に「承知しました」等の定型文が含まれる」** | プロンプトの【制約事項】に「導入・結びの文言は不要」を追加 |
| **「AIが番組情報を参照していない」** | `lib/prompts/system-prompt.ts` の `PROGRAMS` 配列を確認 |
| **「プロンプトを本番DBに反映させたい」** | 下記 **「プロンプトをDBに反映する」** を参照 |

### プロンプトをDBに反映する

- **編集場所**: `docs/prompts/<PROMPT_KEY>.md`（例: `docs/prompts/MINUTES.md`）
- **反映コマンド**: `docs/prompts/*.md` を編集したあと、以下で本番DBへ反映する。

```bash
node scripts/prompts/update-from-doc.mjs <PROMPT_KEY> "変更理由"
# 例: node scripts/prompts/update-from-doc.mjs MINUTES "議題別まとめの時系列化"
```

- 別ファイルを指定する場合: `--file path/to/file.md` を末尾に付与。
- 詳細: [`docs/specs/api-integration/system-prompt-management.md`](./docs/specs/api-integration/system-prompt-management.md)
...
| **Single Source of Truth** | `docs/prompts/*.md`（編集後、上記スクリプトでDBへ反映） |
