...
async function main() {
  ...
  const u1 = r1.usage;
  console.log(
    `input=${u1.input_tokens} (cached=${u1.input_tokens_details?.cached_tokens}) output=${u1.output_tokens} (reasoning=${u1.output_tokens_details?.reasoning_tokens})`,
  );
  ...
  const expected1 = (nonCached1 * 0.2 + cachedTokens1 * 0.05 + u1.output_tokens * 0.5) / 1_000_000;
  const actual1 = u1.cost_in_usd_ticks / 10_000_000_000;
  console.log(
    `期待(token only): $${expected1.toFixed(8)}  実測(/10B): $${actual1.toFixed(8)}  差: ${((actual1 / expected1 - 1) * 100).toFixed(1)}%`,
  );
  ...
  const u2 = r2.usage;
  console.log(
    `input=${u2.input_tokens} (cached=${u2.input_tokens_details?.cached_tokens}) output=${u2.output_tokens} (reasoning=${u2.output_tokens_details?.reasoning_tokens})`,
  );
  ...
  const expected2 = (nonCached2 * 0.2 + cachedTokens2 * 0.05 + u2.output_tokens * 0.5) / 1_000_000;
  const actual2 = u2.cost_in_usd_ticks / 10_000_000_000;
  console.log(
    `期待(token only): $${expected2.toFixed(8)}  実測(/10B): $${actual2.toFixed(8)}  差: ${((actual2 / expected2 - 1) * 100).toFixed(1)}%`,
  );

  // テスト3: web_search 1回
  console.log("\n--- テスト3: web_search つき ---");
  const r3 = await callApi("grok-4-1-fast-reasoning", "xAI社の設立年は？", [
    { type: "web_search" },
  ]);
  const u3 = r3.usage;
  console.log(
    `input=${u3.input_tokens} (cached=${u3.input_tokens_details?.cached_tokens}) output=${u3.output_tokens} (reasoning=${u3.output_tokens_details?.reasoning_tokens})`,
  );
  console.log(`cost_in_usd_ticks=${u3.cost_in_usd_ticks}`);
  console.log(
    `server_side_tool_usage_details=${JSON.stringify(u3.server_side_tool_usage_details)}`,
  );
  const cachedTokens3 = u3.input_tokens_details?.cached_tokens ?? 0;
  const nonCached3 = u3.input_tokens - cachedTokens3;
  const tokenCost3 = (nonCached3 * 0.2 + cachedTokens3 * 0.05 + u3.output_tokens * 0.5) / 1_000_000;
  ...
  console.log(
    `期待(token+tool): $${expected3.toFixed(8)}  (token=$${tokenCost3.toFixed(8)} tool=$${webSearchCost3.toFixed(4)})`,
  );
  console.log(
    `実測(/10B):       $${actual3.toFixed(8)}  差: ${((actual3 / expected3 - 1) * 100).toFixed(1)}%`,
  );
  ...
  const u4 = r4.usage;
  console.log(
    `input=${u4.input_tokens} (cached=${u4.input_tokens_details?.cached_tokens}) output=${u4.output_tokens} (reasoning=${u4.output_tokens_details?.reasoning_tokens})`,
  );
  console.log(`cost_in_usd_ticks=${u4.cost_in_usd_ticks}`);
  console.log(
    `server_side_tool_usage_details=${JSON.stringify(u4.server_side_tool_usage_details)}`,
  );
  ...
  const tokenCost4_25pctCache =
    (nonCached4 * 2.0 + cachedTokens4 * 0.5 + u4.output_tokens * 6.0) / 1_000_000;
  ...
  console.log(
    `期待(cached割引なし):   $${(tokenCost4_noCacheDiscount + webSearchCost4).toFixed(8)}`,
  );
  ...
  console.log(
    `1回目: input=${r5a.usage.input_tokens} output=${r5a.usage.output_tokens} cost_ticks=${r5a.usage.cost_in_usd_ticks}`,
  );
  console.log(
    `2回目: input=${r5b.usage.input_tokens} output=${r5b.usage.output_tokens} cost_ticks=${r5b.usage.cost_in_usd_ticks}`,
  );
}
