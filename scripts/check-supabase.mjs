...
const supabaseUrl =
  process.env.NEXT_PUBLIC_SUPABASE_URL || "https://tbzqswctewjgmhtswssq.supabase.co";
const supabaseKey =
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRienFzd2N0ZXdqZ21odHN3c3NxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MTIxNTkyOSwiZXhwIjoyMDg2NzkxOTI5fQ.96QbwF9tEksvuwQrNron97uWMQdDstjHfaSMe5rDDig";
...
async function checkSupabase() {
  ...
  if (chats && chats.length > 0) {
    ...
    chats.forEach((c, i) => {
      console.log(
        `  ${i + 1}. ${c.id} | ${c.agent_type} | ${c.title?.substring(0, 30) || "(no title)"} | ${c.updated_at}`,
      );
    });
  } else {
    console.log("  （データなし）");
