...
const databaseUrl =
  ...
  "postgresql://neondb_owner:npg_I4bJ3BjiFCNX@ep-plain-grass-a10993pu-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require";
    ...
    try {
      ...
      if (chatResult.rows[0].count > 0) {
        ...
        recentChats.rows.forEach((c, i) => {
          console.log(
            `    ${i + 1}. ${c.id} | ${c.agentType} | ${c.title?.substring(0, 30) || "(no title)"} | ${c.updatedAt}`,
          );
        });
      }
    } catch (e) {
    ...
    try {
      ...
      if (userResult.rows[0].count > 0) {
        ...
        users.rows.forEach((u, i) => {
          console.log(
            `    ${i + 1}. ${u.email} | ${u.name || "(no name)"} | ${u.id.substring(0, 8)}...`,
          );
        });
      }
    } catch (e) {
