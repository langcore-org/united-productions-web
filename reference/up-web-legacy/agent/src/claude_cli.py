...
class ClaudeCodeCLI:
    ...
    async def run_completion(
        ...
        try:
            ...
            try:
                # Build SDK options
                # Set max_buffer_size to 100MB to support large PDF files
                options = ClaudeAgentOptions(
                    max_turns=max_turns,
                    cwd=self.cwd,
                ...
                # Enable Skills loading from project directory
                # Skills are loaded from .claude/skills/ relative to cwd (CLAUDE_CWD)
                options.setting_sources = ["project"]
                logger.info("🔧 Skills enabled from project directory")

                # Set system prompt - CLAUDE AGENT SDK FORMAT
                # The SDK accepts:
                # 1. A simple string for complete custom system prompt
                # 2. {"type": "preset", "preset": "claude_code", "append": "..."} to add to Claude Code's default
                if system_prompt:
                    # Use simple string format for custom system prompt
                    # This completely replaces the system prompt with the custom one
                    options.system_prompt = system_prompt
                    logger.info(f"🔧 Using custom system_prompt (length={len(system_prompt)})")
                    logger.debug(f"🔧 System prompt content: {system_prompt[:300]}...")
                else:
                    # Use Claude Code preset to maintain expected behavior
                    options.system_prompt = {"type": "preset", "preset": "claude_code"}
                    logger.info("🔧 Using default claude_code preset (no custom system_prompt)")
                ...
                # Known SDK message attributes to extract (avoids slow dir() iteration)
                SDK_ATTRS = ('type', 'content', 'model', 'stop_reason', 'stop_sequence',
                             'usage', 'id', 'role', 'subtype', 'data', 'message',
                             'session_id', 'total_cost_usd', 'duration_ms', 'num_turns',
                ...
                async for message in query(prompt=prompt, options=options):
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(f"📨 SDK Message #{message_count}: type={type(message).__name__}")

                    # Fast path: if already a dict, yield directly
                    if isinstance(message, dict):
                        yield message
                        continue

                    # Convert SDK object to dict using known attributes only
                    message_dict = {}
                    for attr in SDK_ATTRS:
                    ...
                    # Log tool use only at DEBUG level
                    if logger.isEnabledFor(logging.DEBUG):
                        ...
                        if isinstance(content, list):
                            for block in content:
                                ...
                                if block_type == "tool_use":
                                    tool_name = getattr(block, "name", None) or (block.get("name") if isinstance(block, dict) else "unknown")
                                    logger.debug(f"🔧 Tool detected: {tool_name}")

                    yield message_dict

                logger.debug(f"📬 SDK query completed. Total messages: {message_count}")
