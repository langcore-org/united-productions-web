    ...
    async (messages: LLMMessage[], provider?: LLMProvider): Promise<void> => {
      ...
      try {
        const response = await fetch("/api/llm/stream", {
          ...
          body: JSON.stringify({ messages, provider: targetProvider }),
          signal: abortControllerRef.current.signal,
        });
        ...
        for await (const event of parseSSEStream(reader)) {
          ...
          if (event.type === "content") {
            setStreamState((prev) => ({ ...prev, content: prev.content + event.delta }));
          }
          if (event.type === "done") {
            const usageInfo = {
              inputTokens: event.usage.inputTokens,
            ...
            setStreamState((prev) => ({ ...prev, isComplete: true, usage: usageInfo }));
        ...
        if (err instanceof Error && err.name === "AbortError") {
          ...
          setStreamState((prev) => ({ ...prev, error: errorMessage, isComplete: true }));
          handleError(errorMessage);
        }
      }
    },
